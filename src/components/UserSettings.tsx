...
function ContactsManager() {
  ...
  async function addContact() {
    ...
    const { error } = await supabase.from("whatsapp_contacts").insert({
      restaurant_id: prof.restaurant_id, name: name.trim(), phone: cleaned, is_supplier: isSupplier,
    } as never);
    if (error) return toast.error(error.message);
    setName(""); setPhone(""); setIsSupplier(false);
  ...
  function startEdit(c: { id: string; name: string; phone: string; is_supplier?: boolean | null }) {
    setEditingId(c.id);
    setEditName(c.name);
    setEditPhone(c.phone);
    setEditIsSupplier(!!c.is_supplier);
  }
  ...
  async function saveEdit(id: string) {
    ...
    const { error } = await supabase.from("whatsapp_contacts").update({
      name: editName.trim(), phone: cleaned, is_supplier: editIsSupplier,
    } as never).eq("id", id);
    if (error) return toast.error(error.message);
    setEditingId(null);
    qc.invalidateQueries({ queryKey: ["whatsapp_contacts"] });
  ...
  return (
    <div className="space-y-4">
              ...
              {editingId === c.id ? (
                <div className="space-y-2">
                  ...
                  <label className="flex items-center gap-2 text-sm">
                    <Checkbox checked={editIsSupplier} onCheckedChange={(v) => setEditIsSupplier(!!v)} />
                    É fornecedor (a mensagem do WhatsApp vira um pedido de encomenda)
                  </label>
                </div>
              ) : (
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {(c as { is_supplier?: boolean }).is_supplier && (
                        <span className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-medium text-secondary-foreground">Fornecedor</span>
                      )}
                    </div>
      ...
      <div className="space-y-2 border-t pt-4">
        ...
        <label className="flex items-center gap-2 text-sm">
          <Checkbox checked={isSupplier} onCheckedChange={(v) => setIsSupplier(!!v)} />
          É fornecedor (a mensagem do WhatsApp vira um pedido de encomenda)
        </label>
      </div>
    </div>
